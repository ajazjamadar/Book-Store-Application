...
public class DebitCardProcessor implements PaymentProcessor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DebitCardProcessor.class);

    @Override
    public PaymentResponse processPayment(PaymentRequest request) {
        if (!(request instanceof CardPaymentRequest cardRequest)) {
            return new PaymentResponse(PaymentStatus.FAILED, "Invalid request type for debit card payment");
        }
        ...
        String message = "Debit card payment of $%.2f processed successfully for '%s'. Card: %s"
                .formatted(cardRequest.getAmount(), cardRequest.getBookName(), maskedCard);

        logger.info("Payment SUCCESS - Transaction: {}", transactionId);
        return new PaymentResponse(PaymentStatus.SUCCESS, message, transactionId, cardRequest.getAmount(), "DEBIT_CARD");
    }
